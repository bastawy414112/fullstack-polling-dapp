...
contract SingleElectionVoting {
    ...
    modifier onlyBefore(uint256 _timestamp) {
        require(
            ...
            "Action not allowed after this time."
        );
        _;
    }
    ...
    modifier onlyDuring(uint256 _start, uint256 _end) {
        require(
            block.timestamp >= _start && block.timestamp <= _end,
            "Action only allowed during the election."
        );
        _;
    }
    ...
    modifier onlyAfter(uint256 _timestamp) {
        require(
            block.timestamp > _timestamp,
            "Action only allowed after this time."
        );
        _;
    }
    ...
    ) external onlyAdmin onlyBefore(election.startDate - 1 days) {
        require(
            election.candidateList[_candidateAddress].candidateAddress ==
                address(0),
            "Candidate already added."
        );
        election.candidateList[_candidateAddress] = Candidate(
            _candidateAddress,
            _name,
            _party,
            _image,
    ...
    function addVoter(
        ...
        string memory _name,
        uint256 _age
    ) external onlyAdmin onlyBefore(election.startDate) {
        require(
            election.voterList[_voterAddress].voterAddress == address(0),
            "Voter already added."
        );
    ...
    {
        require(
            election.candidateAddresses.length > 0,
        ...
        for (uint256 i = 0; i < election.candidateAddresses.length; i++) {
            ...
            Candidate storage candidate = election.candidateList[
                candidateAddress
            ];
    ...
    function getCandidates() external view returns (Candidate[] memory) {
        ...
        for (uint256 i = 0; i < numCandidates; i++) {
            candidates[i] = election.candidateList[
                election.candidateAddresses[i]
            ];
        }
